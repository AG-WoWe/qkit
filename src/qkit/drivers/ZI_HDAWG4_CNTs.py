...
class ZI_HDAWG4(Instrument):
    ...
    def __init__(self, name, device_id, disable_everything = False, **kwargs):
        ...
        self.add_parameter('sampling_prescaler_core1', type = int,
    ...
    def _do_get_sampling_prescaler_core1(self):
        logging.info(__name__ +': getting sampling_prescaler')
        exponent = self.daq.getInt('/%s/AWGS/1/TIME' % self._device_id)        
        return 2**exponent
    ...
    def upload_waveform(self, awg_core, wave_dict):
        ...
        for channel in wave_dict.keys():
            ...
            if "Ch" in channel:
                    ...
                    print(len(wave_dict[channel]))
            # Align the wave_dict to 16 samples UNCOMMENT IF PROBLEMS ARISE
        ...
        sequencer += '''while(true){\nwaitDigTrigger(1);\nplayWave(''' + wave_names[:-2] + ''');\nwaitWave();\n}''' #change here waitdigtrigger
        #sequencer += '''while(true){\nplayWave(''' + wave_names[:-2] + ''');\nwaitWave();\n}'''
        #sequencer += '''while(true){\nwaitDigTrigger(1);\nplayWave(''' + wave_names[:-2] + ''');\n}''' #without waitwave, that blocks trigger if wave form AWG is not finished
        #sequencer += '''while(true){\nplayWave(''' + wave_names[:-2] + ''');\n}'''
        if self._console_output == True:
            print(sequencer, "\n")
