...
class ADwin_Pro2_V3_CNTs(Instrument):
    ...
    def __init__(self,
                 ...
                 processpath_main="/home/nanospin/ADwin_files/ramp_input_V3_Pro2.TC1",
                 process_number_triggered=2,
                 process_path_triggered='/home/nanospin/ADwin_files/ADCF_Burst_Event_V3_Pro2_group_avg.TC2',
                 process_number_aquisition=3,
                 process_path_aquisition='/home/nanospin/ADwin_files/ADCF_Burst_Event_Stopp_V3_Pro2_group_avg.TC3',
                 process_number_continuous=4,
                 watch_sampling_f="10kHz",
                 process_path_continuous_2kHz='/home/nanospin/ADwin_files/ADCF_continuous_2kHz_V3_Pro2.TC4',
                 process_path_continuous_10kHz='/home/nanospin/ADwin_files/ADCF_continuous_10kHz_V3_Pro2.TC4',
                 process_path_continuous_100kHz='/home/nanospin/ADwin_files/ADCF_continuous_100kHz_V3_Pro2.TC4',
                 process_path_continuous_1MHz='/home/nanospin/ADwin_files/ADCF_continuous_1MHz_V3_Pro2.TC4',
                 process_path_continuous_4MHz='/home/nanospin/ADwin_files/ADCF_continuous_4MHz_V3_Pro2.TC4',
                 devicenumber=2,
                 bootload=True,
                 global_lower_limit_in_V=-10,
                 global_upper_limit_in_V=10,
        ...
        self.adw = adw.ADwin(DeviceNo = devicenumber, raiseExceptions=1, useNumpyArrays=True) # 1 turns on raiseExceptions
        ...
        #ramping speed safe ports in V/s
        self.add_parameter('ramping_speed_safe_ports', type=float,
            flags=Instrument.FLAG_GETSET,
            minval=0.0, maxval=1)
        ...
        self.add_parameter('group_average_parameter', type=int,
                           flags=Instrument.FLAG_GETSET,
                           channels=(1,), channel_prefix='input%d_',
        ...
        self.add_parameter('delta_t_between_measurement_points_in_ns', type = int,
                           flags=Instrument.FLAG_GETSET,
                           channels=(1,), chaneel_prefix='input%d_',
                           minval=250, maxval=1e9, units='',
                           tags=['sweep'])
    ...
    def get_Lock_In(self, input_port, module_number, output_port, frequency, amplitude, time_average, gain, divider):
        ...
        module_number (INT): 1 or 2, module used for output voltage
        ...
        time_average (FLOAT):time over which the average is performed, typically between 2 and 10 periods, where a period is defined by the LI_frequency
        ...
        if module_number == 1 or module_number == 2:
            channel = (module_number-1) * 8 + output_port
        else: 
            logging.warning(__name__+': Only modules 1 and 2 are assigned to outputs for this ADwin Pro2, please choose compatible module number.')
            raise Exception("Only modules 1 and 2 are assigned to outputs. Change module number to 1 or 2.")
        self.set_Par_78_global_long(channel)


        dc_output_value = self.get_out(module_number, output_port)
    ...
    def get_DCcurrent_LIreal_LIimag(self, input_port, module_number, output_port, frequency, amplitude, time_average, gain, divider):
        ...
        time_average (FLOAT): approx time over which the average is performed, typically between 2 and 10 periods, where a period is defined by the LI_frequency
    ...
    def _do_set_ramping_speed_safe_ports(self, speed):
        '''sets the ramping speed in Volts/seconds for safe ports.
        '''
        logging.info(__name__ +': setting ramping speed for normal ports of process Nr.%s to %f s' % (self.processnumber, speed))
        self.set_FPar_78_global_float(speed)
        
    def _do_get_ramping_speed_safe_ports(self):
        '''gets the ramping speed in Volts/seconds for safe ports
        '''
        self.get_FPar_78_global_float()
              
    def digit_to_volt(self, digit, gate = 1, bit_format=16):
    ...
    def _do_set_out(self, new, channel):
        ...
        if self.get('gate%d_oversampling_state'%channel)==1:
            ...
            #self.set_FPar_67_global_float(value) #for debuuuuug
    ...
    def set_out(self, module, output, voltage):
        '''Allows easier access to set function for many gates.
        '''
        gate = (module-1)*8+output
        self.set('gate%d_out'% gate, voltage)
        
    def get_out(self, module, output):
    ...
    def _activate_ADwin(self, value):
        ...
        if value==1:
            ...
            logging.info(__name__+': get Lock-In')
            self.set_Par_76_global_long(4)
        elif value==5:
            logging.info(__name__+': get Lock-In + DC')
            self.set_Par_76_global_long(5)
        else:
    ...
    def initialize_gates(self, number = 16, reset_to_0:bool=True, lower_limit=0, upper_limit=0, speed=0.2, init_V=None):
...
####################################################################################
    ...
    def _do_set_delta_t_between_measurement_points_in_ns(self, time:int, channel):
        logging.info(__name__ + ': setting time between measurement points in burst sequence to: %d' % time)
        self.delta_t_burst = time
        if time%10 != 0:
            logging.error(__name__ + ': time between each sample needs to be multiple of 10ns!!!')
            raise Exception("time between each sample needs to be multiple of 10ns")
        self.set_Par_23_global_long(time)
    
    def _do_get_delta_t_between_measurement_points_in_ns(self, channel):
        logging.info(__name__ + ': getting time between measurement points in burst sequence')
        time = self.get_Par_23_global_long()
        return time
    ...
    def _do_get_sample_rate_triggered_readout(self, channel):
        rate = 1/(self.number*self.delta_t_burst)*10**9 
        return rate
    
    def _do_set_group_average_parameter(self, number, channel):
         logging.info(__name__ + ': setting order parameter of group average to: %d' %number)
         self.number = number
         self.set_Par_22_global_long(number)
         
    def _do_get_group_average_parameter(self, channel):
         logging.info(__name__ + ': getting order parameter of group average')
         number = self.get_Par_22_global_long()
         return number
    ...
    def _do_set_sample_count_triggered_readout(self, sample_count_without_averaging:int, channel):
        ...
        #sample_count = sample_count_without_averaging * self.triggered_readout_averaging #to use the sampling rate as semicon 
        sample_count = sample_count_without_averaging*self.number
        print(sample_count)
        if (sample_count % 8) != 0:
    ...
    def read_triggered_readout(self):
        ...
        for k in range(size):    #small for loop to remove blips to 0bits (can happen randomly sometimes, probably from errors at reading the data...)
            if data_raw[k] == 0:
                print("There is the pb!")
                if k == 0:
                    for i in range(size):
                        if data_raw[i]==0:
                            pass
                        else:
                            data_raw[k] = data_raw[i]
                else:
                    data_raw[k] = data_raw[k-1]
        ...
        #print('data_triggered_readout', data_triggered_readout)
        ...
        if self.check_finished_triggered_readout()==0:
