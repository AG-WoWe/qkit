...
##################################################
##################################################
''' THIS VERSION HAS BEEN Modified BY IOANNIS KARAPATZAKIS,
			IT MAY NOT WORK WITH MARKERS!!!!!!'''
##################################################
##################################################
##################################################
...
class AWG70000A(VisaInstrument):
    ...
    def __init__(self, name: str, address: str, num_channels: int,
                 waveform_dir: str="\\Users\\OEM\\Documents",
                 timeout: float=60, **kwargs) -> None:
        """
        Args:
        ...
        self.wfmxFileFolder = waveform_dir
        self.seqxFileFolder = waveform_dir
    ...
    def play(self, wait_for_running: bool=True, timeout: float=10) -> None:
        ...
        Args:
            ...
            timeout: The maximal time to wait for the instrument to play.
                Raises an exception if this time is reached.
        """
    ...
    def makeWFMXFile(data: np.ndarray, amplitude: float) -> bytes:
        ...
        chunk = int(5e7)
        split = round(len(data)/chunk) + 1
		
        for i in range(split):
            wfmx +=  AWG70000A._makeWFMXFileBinaryData(data[chunk*i:chunk*(i+1)], amplitude)

        return wfmx

    def makeWFMXFile_ORIGINAL_VERSION(data: np.ndarray, amplitude: float) -> bytes:
        """
        Compose a WFMX file
        ...
        wfmx_hdr_str = AWG70000A._makeWFMXFileHeader(num_samples=N,
                                                     markers_included=markers_included)
        wfmx = bytes(wfmx_hdr_str, 'ascii')
        wfmx +=  AWG70000A._makeWFMXFileBinaryData(data, amplitude)
    ...
    def _sendBinaryFile_mod(self, binfile: bytes, filename: str,
                        path: str, overwrite: bool=True) -> None:
        """
        Send a binary file to the AWG's mass memory (disk).
        ...
        """
        chunk = int(9e8)
        split = round(len(binfile)/chunk + 0.5)
        if len(binfile) > chunk-1:
            print('Data has to be splitted into', split, 'chunks\n')
        binfile_ = []

        name = filename
        for i in range(split):
            if split > 1:
                filename = name.rstrip('.wfmx') + 'm_chunk_' + str(i) + '.wfmx'
            name_str = 'MMEMory:DATA "{}"'.format(filename).encode('ascii')
            len_file = len(binfile[chunk*i:chunk*(i+1)])
            len_str = len(str(len_file))  # No. of digits needed to write length
            size_str = (',#{}{}'.format(len_str, len_file)).encode('ascii')
            binfile_.append(name_str + size_str + binfile[chunk*i:chunk*(i+1)])

        # IEEE 488.2 limit on a single write is 999,999,999 bytes
        # TODO: If this happens, we should split the file
        # if len(binfile) > 1e9-1:
            # raise ValueError('File too large to transfer')
        ...
        if overwrite:
            ...
            self.log.debug(f'Pre-deletion finished with return code {resp}')
        for i in range(split):
            self.visa_handle.write_raw(binfile_[i])
    ...
    def loadWFMXFile(self, filename: str, path: str=None) -> None:
        ...
        try:
            ...
            print('Loading WFMX failed.')
            self.loadWFMXFile(filename, path)
    ...
    def _makeWFMXFileBinaryData(data: np.ndarray, amplitude: float) -> bytes:
        ...
        wfm = None
        markers = None
        binary_wfm += binary_marker
        gc.collect()
        struct._clearcache() #Necesarry to clear chache since struct somehow does shit here, Memoryleak
        return binary_wfm
