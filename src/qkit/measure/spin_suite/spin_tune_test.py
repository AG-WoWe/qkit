...
from qkit.measure.semiconductor.utils.multiplexer_test import Sequential_multiplexer
...
class Tuning(mb.MeasureBase):
    ...
    Methods
    -------
    register_measurement(name, unit, nodes, modes, get_tracedata_func, *args, **kwargs):
    ...
    set_node_bounds(measurement, node, mode, bound_lower, bound_upper):
    ...
    def register_measurement(self, name, nodes, modes, get_tracedata_func, *args, **kwargs):
        ...
        *args, **kwargs:
        ...
        None
        """
        self.multiplexer.register_measurement(name, nodes, modes, get_tracedata_func, *args, **kwargs)
        self.name_meas = name
    ...
    def set_node_bounds(self, measurement, node, mode, bound_lower, bound_upper):
        """
        Sets the upper and the lower bounds for a registered measurement node.
        ...
        self.watchdog.register_node(f"{measurement}.{node}.{mode}", bound_lower, bound_upper)
    ...
    def _prepare_empty_container(self, modes):
        sweepy = {}
        for name, measurement in self.multiplexer.registered_measurements.items():
            if measurement["active"]:
                for node in measurement["nodes"]:
                    sweepy[f"{name}.{node}.{modes}"] = []
                  #  if modes == "trace":
                  #      sweepy[f"{name}.{node}.trace"] = []
                 #   elif modes == "retrace":
                  #      sweepy[f"{name}.{node}.trace"] = []
                  #      sweepy[f"{name}.{node}.retrace"] = []
                  #  elif modes == "difference":
                  #      sweepy[f"{name}.{node}.trace"] = []
                  #      sweepy[f"{name}.{node}.retrace"] = []
                  #      sweepy[f"{name}.{node}.difference"] = []
        return sweepy
    ...
    #def _append_vector(self, latest_data, container, modes, direction):
     #   for name, measurement in self.multiplexer.registered_measurements.items():
     #           for node in measurement["nodes"]:
     #               for values in latest_data.items():
      #                  container[f"{name}.{node}.{modes}"].append(values[::direction])
    ...
    def measure_hyst1D(self):
        ...
        pb = Progress_Bar(len(self._x_parameter.values) * self.multiplexer.no_active_nodes_and_modes)
        
        dsets = self.multiplexer.prepare_measurement_datasets([self._x_parameter])
        self._prepare_measurement_file(dsets)
        self._open_qviewkit()
        
        a = self._data_file.add_view("Hysteresis", x = self._datasets[self._y_parameter.name], y = self._datasets["Block.1"])
       # a.add(x = self._datasets[self._y_parameter.name], y = self._datasets[f"{name}.{node}.retrace"])
        print(a)
    ...
    def measure1D(self, modes, data_to_show = None):
        """
        ...
        assert self._x_parameter, f"{__name__}: Cannot start measure1D. x_parameters required."
        self._measurement_object.measurement_func = "%s: measure1D" % __name__
        #pb = Progress_Bar(len(self._x_parameter.values) * self.multiplexer.no_active_nodes_trace())
        ...
        try:
            for x_val in self._x_parameter.values:
                ...
                latest_data = self.multiplexer.measure(modes)
    ...
    def measure2D(self, modes, data_to_show = None):
        """
        Starts a 2D - measurement, with y being the inner and x the outer loop coordinate.
        
        Parameters
        ----------
        modes: trace, retrace (includes trace) or difference (includes trace and retrace) of the y-parameter
        name_meas: name of the registered measurement that will be started
        data_to_show : List of strings, optional
            Name of Datasets, which qviewkit opens at measurement start.
        """
        ...
        if modes == "trace":
           a = 0# ab = Progress_Bar(len(self._x_parameter.values) * len(self._y_parameter.values) * self.multiplexer.no_active_nodes_trace())
        else:
            pb = Progress_Bar(len(self._x_parameter.values) * len(self._y_parameter.values) * self.multiplexer.no_active_nodes_with_retrace())
        ...
        try:
            ...
            for x_val in self._x_parameter.values:             
                ...
                qkit.flow.sleep(self._x_parameter.wait_time)
                sweepy = self._prepare_empty_container('trace')
                y_direction = 1
                y_direction_re = -1
                if modes == "trace":
                    ...
                    for y_val in self._y_parameter.values[::y_direction]:                    
                      ...
                      #  ab.iterate(addend = len(latest_data))
                    ...
                    sweepy_retrace = self._prepare_empty_container('retrace')
                    for y_val in self._y_parameter.values[::y_direction]:                    
                        self._y_parameter.set_function(y_val)
                        qkit.flow.sleep(self._y_parameter.wait_time)
                        latest_data = self.multiplexer.measure('trace')
                    ...
                    for y_val in self._y_parameter.values[::y_direction_re]:                    
                        self._y_parameter.set_function(y_val)
                        qkit.flow.sleep(self._y_parameter.wait_time)
                        latest_data2 = self.multiplexer.measure('retrace')
                        self._append_value(latest_data2, sweepy_retrace)
                        pb.iterate(addend = len(latest_data2))
                    self._append_vector(sweepy_retrace, self._datasets, direction = y_direction_re)
                    ...
                    if modes == 'difference':
                        sweepy_diff = self._prepare_empty_container('difference')
                        for node in self.multiplexer.registered_measurements[self.name_meas]["nodes"]:
                                 x = sweepy[f"{self.name_meas}.{node}.trace"]
                                 y = sweepy_retrace[f"{self.name_meas}.{node}.retrace"]
                                 y.reverse()
                                 for i in range(len(self._y_parameter.values)):
                                     sweepy_diff[f"{self.name_meas}.{node}.difference"].append(x[i] - y[i])
                        self._append_vector(sweepy_diff, self._datasets, direction = y_direction)
                        #pb.iterate(addend = len(latest_data))
                        if self.watchdog.stop:
                            warn(f"{__name__}: {self.watchdog.message}")
                            break
                    for node in self.multiplexer.registered_measurements[self.name_meas]["nodes"]:
                       ...
                       # print(self._coordinates[self._y_parameter.name])
                       # print(self._datasets[f"{self.name_meas}.{node}.trace"])
                        a = self._data_file.add_view(f"Trace_retrace_{node}", x = self._coordinates[self._y_parameter.name], y = self._datasets[f"{self.name_meas}.{node}.trace"])
                        a.add(x = self._coordinates[self._y_parameter.name], y = self._datasets[f"{self.name_meas}.{node}.retrace"])
    ...
    def measure3D(self, modes, data_to_show = None):
        """
        ...
        pb = Progress_Bar(len(self._x_parameter.values) * len(self._y_parameter.values) * len(self._z_parameter.values) * self.multiplexer.no_active_nodes_and_modes(modes))
