# -*- coding: utf-8 -*-
"""
Created on Tue Apr 11 17:11:47 2023

@author: Thomas Koch
"""
import time
from qkit.measure.spin_suite.backends.manipulation_backends.MA_backend_base import MA_backend_base

class Moku_Pro_ZI_UHFLI_backend(MA_backend_base):
    def __init__(self, Moku_Pro, UHFLI):
        self.register_channel("Ch1", "V")
        self.register_channel("Ch2", "V")
        self.register_channel("Ch3", "V")
        self.register_channel("Ch4", "V")
        
        self.register_channel("Trig1", "V")
        self.register_channel("Trig2", "V")
        self.register_channel("Trig3", "V")
        self.register_channel("Trig4", "V")
        
        self.moku = Moku_Pro
        self.locke = UHFLI
        
    def Ch1_get_sample_rate(self):
        rate = self.moku.get_sampling_rate()*1e-9
        return rate
    
    def Ch2_get_sample_rate(self):
        rate = self.moku.get_sampling_rate()*1e-9
        return rate
    
    def Ch3_get_sample_rate(self):
        rate = self.moku.get_sampling_rate()*1e-9
        return rate
    
    def Ch4_get_sample_rate(self):
        rate = self.moku.get_sampling_rate()*1e-9
        return rate
    
    def Trig1_get_sample_rate(self):
        rate = self.moku.get_sampling_rate()*1e-9
        return rate

    def Trig2_get_sample_rate(self):
        rate = self.moku.get_sampling_rate()*1e-9
        return rate

    def Trig3_get_sample_rate(self):
        rate = self.moku.get_sampling_rate()*1e-9
        return rate

    def Trig4_get_sample_rate(self):
        rate = self.moku.get_sampling_rate()*1e-9
        return rate
        
    def run(self):
        # Starte Manipulation (Moku-Ausgänge an)
        # Mindestens den verwendeten Trigger-Ausgang einschalten:
        self.moku.enable_output(1)     # wenn dein Trigger auf OUT1 liegt
        # optional, falls du mehrere Ausgänge verwendest:
        # for ch in (1,2,3,4): self.moku.enable_output(ch)

    def stop(self):
        # Stoppe Manipulation (Moku-Ausgänge aus), damit beim Arming kein Trigger kommt
        self.moku.disable_output(1)
        # optional:
        # for ch in (1,2,3,4): self.moku.disable_output(ch)


    # def run(self):
        # self.locke.start_triggered_readout()

    # def stop(self):
        # self.locke.stop_triggered_readout()
        
    def load_waveform(self, wave_data):
        self.moku.configure_outputs(wave_data, triggered = False)

    
    def decode(self, pulse_obj, parameters):
        self.moku._seq_prog = pulse_obj.create_program(parameters = parameters)
        self.moku._frequency = 1/(int(self.moku._seq_prog.duration)/1e9)